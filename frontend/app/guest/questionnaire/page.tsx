...
const QuizPage = () => {
    ...
    const getResults = () => {
        return selectedAnswers.map((answersForCategory, categoryIndex) => {
            const totalA = answersForCategory.reduce(
                (sum, answer) => sum + answer.a,
                0
            );
            const totalB = answersForCategory.reduce(
                (sum, answer) => sum + answer.b,
                0
            );
            return {
                        ...
                        {getResults().map((result, index) => (
                            ...
                            >
                                ...
                                <p className="text-gray-600">
                                    Answers A:{" "}
                                    <span className="font-semibold">{result.totalA}</span>
                                </p>
                                <p className="text-gray-600">
                                    Answers B:{" "}
                                    <span className="font-semibold">{result.totalB}</span>
                                </p>
                            </div>
                        ))}
                    </div>
